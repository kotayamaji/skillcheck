...
<head>
<meta charset="UTF-8">
<title><fmt:message key="title.students.search.result" /></title>
...
<body>
	...
	<form action="/students/searchResult" method="post">
		<table>
			<thead>
				<tr>
					<th><fmt:message key="lbl.student.name" /></th>
					<th><fmt:message key="lbl.grade" /></th>
					<th>出身</th>
					<th>専攻</th>
			...
			<tbody>
				<c:forEach items="${sessionInfo.studentSearchResult}" var="student">
					<tr>
						...
						<td>${student.grade}</td>
						<td>${fn:escapeXml(student.hometown)}</td>
						<td>${fn:escapeXml(student.majorName)}</td>
						<c:if test="${sessionInfo.loginUser.roleId eq 1 }">
							<td>
	...
	<div>
		<a href="/students/search"><fmt:message key="btn.back" /></a>
	</div>
</body>
</html>

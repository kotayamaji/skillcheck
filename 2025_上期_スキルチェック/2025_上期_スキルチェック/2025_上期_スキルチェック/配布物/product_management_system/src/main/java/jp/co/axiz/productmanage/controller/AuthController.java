...
public class AuthController {
    ...
    public AuthController(MessageSource messageSource,HttpSession session, UserService userInfoService) {
     ...
     */
    @PostMapping("/auth")
    public String auth(@Validated @ModelAttribute("loginForm") LoginForm form,BindingResult bindingResult, Model model) {
        ...
        List<categories> categories = userInfoService.categories();
        categories all = new categories(0, "すべて");
        categories.add(0,all);

        // ログインユーザ情報をセット
        sessionInfo.setLoginUser(user);
        sessionInfo.setCategoryList(categories);
